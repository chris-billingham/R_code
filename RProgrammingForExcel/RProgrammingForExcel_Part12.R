...
#
# This R source code file corresponds to video 12 of the YouTube series
# "R Programming for Excel Users" located at the following URL:
#      https://youtu.be/Tfuce6UvsAQ     
#
...
# Create a list of the various objects we've built so far
my_list <- list(star_wars = star_wars,
                birthplaces = birthplaces,
                mtcars = mtcars,
                my_bigger_vector = my_bigger_vector)
str(my_list)

# Various ways to access a list - start with a list of the first element
str(my_list[1])
str(my_list[[1]])

my_list[1]
my_list[[1]]
# A list of the first two elements
str(my_list[1:2])

# A list of the first and third elements
str(my_list[c(1, 3)])

# Access the first element of the list directly by index
str(my_list[[1]])

# Access the first element of the list by name
str(my_list$star_wars)

# Slice elements of the list and view in RStudio
View(my_list[[1]][1:5,])
View(my_list$mtcars[1:10,])


# Create a list with a vector of characters (strings) and a 
# list of characters.
non_force_users <- list(rebellion = c("Han Solo", "Chewbacca", "Lando Calrissian"),
                        empire = list("Grand Moff Tarkin", "General Veers", "Captain Needa"))
non_force_users


# Add the new list to our existing list.
# NOTE - This works, but is not effecient. Using sparingly!
my_list$non_force_users <- non_force_users
str(my_list)


# Access our list within a list within a list!
my_list[[5]][[2]][[2]]
my_list$non_force_users$empire[[3]]


# Many R functions return lists, often you want to transform a list into
# a vector to make your life easier!
empire <- unlist(my_list$non_force_users$empire)
str(empire)
empire
